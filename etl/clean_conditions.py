...
# Keep the full UUIDs so these still point at encounters.id and patients.
df_conditions["PATIENT"] = df_conditions["PATIENT"].str.strip().str.lower()
df_conditions["ENCOUNTER"] = df_conditions["ENCOUNTER"].str.strip().str.lower()


# START and STOP are plain dates with no time part, so keep them as dates.
# Anything unparseable is coerced to NaT rather than crashing the run.
...
duplicate_rows = df_conditions.duplicated(
...
df_conditions.to_csv(clean_dir / "conditions.csv", index=False)
