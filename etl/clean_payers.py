...
# Keep the full UUID. payers.Id is a uuid column in Postgres and is the target
# of the foreign key from encounters.PAYER, so it must not be trimmed.
df_payers["Id"] = df_payers["Id"].str.strip().str.lower()
...
duplicate_ids = df_payers["Id"].duplicated().sum()
...
df_payers.to_csv(clean_dir / "payers.csv", index=False)
